...
function sanitizeAI(data: any) {
  ...
  if (Array.isArray(data.technicalQuestion)) {
    data.technicalQuestion = data.technicalQuestion.map((q: any) => {
      if (typeof q === "string") {
  ...
  if (Array.isArray(data.behaviouralQuestion)) {
    data.behaviouralQuestion = data.behaviouralQuestion.map((q: any) => {
      if (typeof q === "string") {
        const parsed = tryParseJSON(q);
        if (parsed && typeof parsed === "object" && parsed.question) return parsed;
        return { question: q, intention: "Auto-generated intention", answer: "Auto-generated answer" };
  ...
  if (Array.isArray(data.skillGap)) {
    data.skillGap = data.skillGap.map((s: any) => {
      if (typeof s === "string") {
        const parsed = tryParseJSON(s);
        if (parsed && typeof parsed === "object" && parsed.skill) return parsed;
  ...
  if (Array.isArray(data.preparationPlan)) {
    data.preparationPlan = data.preparationPlan.map((p: any, i: number) => {
      if (typeof p === "string") {
